...
class TestUnlinkAccount:
    @pytest.mark.asyncio
    async def test_reject_google_unlink(self):
        from intentkit.utils.error import IntentKitAPIError

        from app.team.user import UnlinkAccountRequest, unlink_account

        body = UnlinkAccountRequest(provider="google")
        with pytest.raises(IntentKitAPIError) as exc_info:
            await unlink_account(body=body, user_id="user-123")
        assert exc_info.value.status_code == 400
        assert exc_info.value.key == "CannotUnlink"

    @pytest.mark.asyncio
    async def test_require_google_for_evm_unlink(self, evm_identities):
        from intentkit.utils.error import IntentKitAPIError

        from app.team.user import UnlinkAccountRequest, unlink_account

        body = UnlinkAccountRequest(provider="evm")
        with (
            patch(
                "app.team.user.get_user_identities",
                new_callable=AsyncMock,
                return_value=evm_identities,
            ),
            pytest.raises(IntentKitAPIError) as exc_info,
        ):
            await unlink_account(body=body, user_id="user-123")
        assert exc_info.value.status_code == 400
        assert exc_info.value.key == "GoogleRequired"

    @pytest.mark.asyncio
    async def test_successful_evm_unlink(self, both_identities):
        import json
        ...
        with (
            ...
            patch(
                "app.team.user.unlink_identity",
                new_callable=AsyncMock,
                return_value=True,
            ),
            patch(
                "app.team.user.UserUpdate",
            ) as mock_user_update,
            patch(
                "app.team.user.invalidate_user_cache",
                new_callable=AsyncMock,
            ),
        ):
            mock_update_instance = AsyncMock()
            mock_user_update.model_validate.return_value = mock_update_instance

            response = await unlink_account(body=body, user_id="user-123")
